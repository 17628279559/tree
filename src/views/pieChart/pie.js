...
let pie = (id, svgRef, data, options) => {
    ...
    let domain_gradient_color = []
    for (let i = 0; i < settings.gradient_color.length; i++) {
        domain_gradient_color.push(data.length * (i / (settings.gradient_color.length - 1)))
    }
    console.log(data.length);
    let colorScale = d3.scaleLinear().domain(domain_gradient_color).range(settings.gradient_color),
